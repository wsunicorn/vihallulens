...
| **ViWikiFC** | Đối chứng ngoài + thí nghiệm ảo giác ngoại lai | `viwikifc_train.csv`, `viwikifc_dev.csv`, `viwikifc_test.csv` | Duy nhất có bằng chứng cho cả nhãn NEI (100% nguyên văn, xác nhận ở T11). Tổng thể 20.918/20.919, một dòng hỏng ở nguồn. Tập test có nhãn nhưng **rò rỉ 100 % ngữ cảnh với train**. `pairID` không phải khóa chính. **Đo ở T13: chỉ 67 % nhãn NEI thật sự là ngoại lai**, kappa 0,505. |
| **ViFactCheck** | Dự phòng, thí nghiệm chuyển miền | `vifactcheck_train.parquet`, `vifactcheck_dev.parquet`, `vifactcheck_test.parquet` | Chỉ làm nếu tuần 14–15 còn thời gian. Bằng chứng là **nhiều câu rời nhau ghép lại** nên một cặp offset không biểu diễn được; 59 % là tỷ lệ bằng chứng chỉ có một mảnh, đo ở T12. `annotation_id` không phải khóa chính. |
