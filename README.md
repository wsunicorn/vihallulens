...
Trang chủ ở `http://127.0.0.1:8000/` vừa giới thiệu đề tài (cơ chế, số liệu, phần "nói thẳng", cú Lens dõi theo con trỏ) vừa là công cụ: dán ngữ cảnh, câu hỏi, câu trả lời, bấm Chấm — ngữ cảnh được tô màu theo tỷ trọng chú ý mỗi đoạn nhận được, kèm điểm rủi ro và xác suất ba lớp. HTML + JS thuần, không framework. Không có mô hình (máy không GPU, hoặc chép `serve/static/` lên GitHub Pages) thì trang tự vào **chế độ phát lại** với kết quả Kaggle 11/09/2026 từ `replay.json` — sinh bằng `python scripts/build_replay.py`. Các route theo `docs/SPEC.md` §2.6, tài liệu tương tác ở `http://127.0.0.1:8000/docs`:
